...
Function Main(  )
    *Ferase("Agenda.fdb")
    ...
    if !File("Agenda.fdb") .and. !CreateDatabase()
        Return .F.
    endif
    
    //Include do BD, realocado conteudo para ca por conta da memvar...
    *#Include <Agenda.DB>
    Private oFbServer//Variáveis globais de conexão
    oFbServer:=TFBServerIma():New('Agenda.fdb','SYSDBA','',3) //Cria o objeto MySQL
    ...
    Load window Agenda as Main
        Main.OnRelease := {||oFbServer:Close()}
    Main.Center
...
Static function SalvaContato()
    ...
    if lNovo
        ...
        Rs.Sql:="Select First 1 codigo from agenda order by codigo desc"
...
Static Function Main_oCidade_Onlistdisplay( )
    ...
    Local Rs
    Rs.New()
    Rs.SQL := "Select First 100 codcidade,nomecidade from cidade where nomecidade like '"+Main.oCidade.DisplayValue+"%'"
    if !Empty(cUF)

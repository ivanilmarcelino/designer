...
function CreateDatabase()
    Local cn, cSQL,lRet
    FBCreateDB( "Agenda.fdb", "SYSDBA", "", 1024, "WIN1252", 3,"WIN_PTBR")
    *FBCreateDB( "Agenda.fdb", "SYSDBA", "", 1024, "ISO8859_1", 3,"PT_BR")
    if !File("Agenda.fdb")
        Return .F.
    endif
    cn:=TFBServerIma():New('Agenda.fdb','SYSDBA','',3) //Cria o objeto MySQL
    if cn:Neterr() //verifica se ouve erro na abertura do banco
        MsgStop('Erro ao conectar banco de dados Mysql' + cn:Error())
        Return .F.
    Endif    
    **********************************
...
Static function CargadeDados(cn)
    ...
    For nk=1 to len(aList)
        ...
        While !oFile:Eof()
            ...
            cSQL:=hb_oemtoAnsi(HB_UTF8ToStr(oFile:Get()))
            if !Empty(cSQL)

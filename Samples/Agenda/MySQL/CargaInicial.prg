...
function CreateDatabase(cn)
    Local  cSQL,lRet
    cn:Execute("Create Database agenda;")
    cn:SelectDB('agenda')
    if cn:Neterr() 
        MsgStop('Erro ao criar o database o Database !!' + 'agenda:'+cn:Error())
        Return .F.    
    endif
    
    *latin1_swedish_ci: Não distingue letras minúsculas e maiúsculas e nem acentuação
    cn:Execute("ALTER DATABASE `agenda` CHARSET = Latin1 COLLATE = latin1_swedish_ci;")

...
Function Main(  )
    ...
    *#Include <Agenda.DB>
    Private oPgServer//Variáveis globais de conexão
    ...
    if oPgServer:Neterr() //verifica se ouve erro na abertura do banco
        ...
        Return .F.
    Endif
    *oPgServer:Execute("Drop table agenda")
    *oPgServer:Execute("Drop table cidade")
    *oPgServer:Execute("Drop table tbuf")
    
    IF !oPgServer:TableExists("agenda")
        IF !CreateDatabase(oPgServer)
            Return .F.
        endif
    endif
    ...
    Load window Agenda as Main
        Main.OnRelease := {||oPgServer:Close()}
    Main.Center
